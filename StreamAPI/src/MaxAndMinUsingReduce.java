...
public class MaxAndMinUsingReduce {
	...
	private static int getMaxValue(List<Integer> ll) {
		return ll.stream().reduce(Integer.MIN_VALUE, (a, b) -> a > b ? a : b);

	}

	private static int getMinValue(List<Integer> ll) {
		return ll.stream().reduce(Integer.MAX_VALUE, (a, b) -> b > a ? a : b);
	}

}

// by taking the integer highest And lowest value makes the function -ive elements proof if we take 
// 0 in initial value then result will come wrong
